...
describe("startFeed route envelopes", () => {
  ...
  it("requests bounded trail backfill for the selected aircraft", () => {
    vi.useFakeTimers();
    ...
    useIdentStore.getState().ingestAircraft({
      now: 1_766_576_400,
      aircraft: [{ hex: "abc123", lat: 34.1, lon: -118.2 }],
    });
    useIdentStore.getState().select("abc123");
    useIdentStore.getState().select("abc123");

    expect(wsHarness.instances[0].sent).toEqual([
      {
        type: "trails.backfill",
        id: "trail-1",
        since: 1_766_569_200_000,
        until: 1_766_575_800_000,
        hex: ["abc123"],
      },
    ]);
    stop();
