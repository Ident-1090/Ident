...
function dispatch(env: Envelope): void {
  ...
  switch (env.type) {
    ...
    case "replay.availability": {
      ...
      const reportedFrom = env.data.fromEpochSec;
      const reportedTo = env.data.toEpochSec;
      const reportedBlockSec = env.data.blockSec;
      // Envelope reports seconds; store keeps ms. Convert before compare.
      const countChanged =
        typeof reportedBlockCount === "number" &&
        reportedBlockCount !== knownBlockCount;
      const fromChanged =
        typeof reportedFrom === "number" &&
        reportedFrom * 1000 !== store.replay.availableFrom;
      const toChanged =
        typeof reportedTo === "number" &&
        reportedTo * 1000 !== store.replay.availableTo;
