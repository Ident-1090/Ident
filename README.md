...
separate history producer. The defaults retain two hours in memory, send the
most recent ten minutes when a browser connects, sample each aircraft at most
once every five seconds, and write a compressed restart cache once a minute.

```sh
IDENT_TRAILS_MEMORY_WINDOW_SEC=7200
IDENT_TRAILS_SNAPSHOT_WINDOW_SEC=600
IDENT_TRAILS_SAMPLE_INTERVAL_SEC=5
IDENT_TRAILS_BACKFILL_MAX_WINDOW_SEC=7200
IDENT_TRAILS_BACKFILL_MAX_POINTS=100000
IDENT_TRAILS_RESTART_CACHE=true
IDENT_TRAILS_RESTART_CACHE_DIR=/var/cache/ident
IDENT_TRAILS_RESTART_CACHE_INTERVAL_SEC=60
```

`IDENT_TRAILS_MEMORY_WINDOW_SEC` controls how much recent trail data `identd`
keeps available. `IDENT_TRAILS_SNAPSHOT_WINDOW_SEC` controls the initial
WebSocket payload for a newly opened browser. Backfill requests are capped by
`IDENT_TRAILS_BACKFILL_MAX_WINDOW_SEC` and `IDENT_TRAILS_BACKFILL_MAX_POINTS` so one
client cannot ask the service to serialize unbounded trail history.
